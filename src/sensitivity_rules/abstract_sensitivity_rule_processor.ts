...
export abstract class AbstractSensitivityRuleProcessor {
    ...
    static initializeRules(): Rule[] {
        const rules: Rule[] = JSON.parse(fs.readFileSync(AbstractSensitivityRuleProcessor.SENSITIVITY_RULES_FILE).toString()).rules.map((n: any) => { return Object.assign(new Rule, n) });
